...
export default function ProductMain({ product }: { product: any }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-stretch mb-20">
      ...
      {/* LEFT: Image Card with Functional Wishlist Toggle */}
      <Reveal className="h-full">
        <div className="relative h-full min-h-[400px] bg-white rounded-[32px] overflow-hidden shadow-xl border border-black/5">
          ...
          <button 
            ...
            className={`absolute top-6 right-6 w-12 h-12 rounded-full bg-white/80 backdrop-blur-md flex items-center justify-center transition-colors shadow-lg ${
              isFav ? "text-[#FF6B35]" : "text-black/20 hover:text-[#FF6B35]"
            }`}
          >
            <Heart size={20} className={isFav ? "fill-[#FF6B35]" : ""} />
          </button>
        </div>
      </Reveal>
